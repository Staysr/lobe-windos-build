const o=(n,a,e)=>{let t="";return e?.length&&(t=`

Current conversation context:
${e.map(s=>`${s.role}: ${s.content}`).join(`
`)}`),{max_tokens:100,messages:[{content:`You are an autocomplete engine for a chat input box. The user is composing a message to send to an AI assistant. Predict and complete what the USER is typing. Output ONLY the missing text to insert at the cursor.
...
"Let me " → "explain that for you" ← WRONG: assistant offering to explain${t}`,role:"system"},{content:`Before cursor: "${n}"
After cursor: "${a}"`,role:"user"}]}};export{o as c};
